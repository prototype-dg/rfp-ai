...
async function processQueueMessage(msg, env) {
    const { rfpId, vendorId, r2Key, filename, label } = msg;
    ...
    if (isPostScriptGarbage(extracted)) {
        ...
        const proposal = await db.prepare(`SELECT id FROM proposals WHERE rfp_id=? AND vendor_id=? ORDER BY id DESC LIMIT 1`).bind(rfpId, vendorId).first();
        if (proposal) {
  ...
  `).bind(rfpId, vendorId).first();
    ...
    WHERE id=?
  `).bind(fields.executive_summary || proposal.executive_summary, fields.key_strengths || proposal.key_strengths, fields.budget_amount ?? proposal.budget_amount, fields.budget_currency || 'AED', fields.timeline_months ?? proposal.timeline_months, mergedText, proposedDuration || proposal.proposed_duration, `r2://${r2Key}`, proposal.id).run();
    ...
    try {
        ...
        const total = evalResult.scoringDetails && evalResult.scoringDetails.length > 0
            ? Math.round(evalResult.scoringDetails.reduce((s, c) => s + (c.weighted !== undefined ? Number(c.weighted) : Number(c.weight || 0) * Number(c.score || 0) / 100), 0))
    ...
    `).bind(rfpId, proposal.id, vendorId, evalResult.scores.business, evalResult.scores.technical, evalResult.scores.financial, evalResult.scores.experience, total, evalResult.summary, JSON.stringify(evalResult.scoringDetails)).run();
