...
exports.profileEmailHtml = profileEmailHtml;
exports.profilePdfBodyHtml = profilePdfBodyHtml;
 ...
 */
function profileEmailHtml(opts) {
 ...
 */
function profilePdfBodyHtml(opts) {
    const p = (0, index_1.getActiveProfile)();
    if (p.id === 'andersen') {
        return andersenPdfBodyHtml({ bodyHtml: opts.bodyHtml });
    }
    // ── CPC PDF: background image letterhead ──────────────────────────────────
    const { contentPaddingTop, contentPaddingBottom, contentPaddingLeft, contentPaddingRight, fontFamily, bodyColor, accentBorderColor, footerText, footerColor, letterheadBgPath } = p.pdf;
