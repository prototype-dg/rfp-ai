...
async function seedVendors(db) {
    ...
    const meta = await db.prepare('SELECT value FROM app_meta WHERE key=?').bind('vendor_seed_version').first();
    ...
    for (const v of vendors) {
        await db.prepare(`INSERT INTO vendors (name, category, country, size, contact_name, contact_email, specializations, certifications, erp_experience)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(v.name, v.category, v.country, v.size, v.contact_name, v.contact_email, v.specializations, v.certifications, v.erp_experience).run();
    }
