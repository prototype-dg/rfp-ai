...
async function runInlineOcr(pdfUrl, env, maxPages = 100) {
    const apiKey = env?.GOOGLE_VISION_API_KEY || globalThis.GOOGLE_VISION_API_KEY || '';
    if (!apiKey) {
...
exports.apiRouter.get('/stats', async (c) => {
    try {
        ...
        const [totalRfps, activeRfps, awardedRfps, totalVendors, totalProposals, totalEmails] = await Promise.all([
            db.prepare('SELECT COUNT(*) as cnt FROM rfps').first(),
            db.prepare("SELECT COUNT(*) as cnt FROM rfps WHERE stage NOT IN ('draft','awarded')").first(),
            db.prepare("SELECT COUNT(*) as cnt FROM rfps WHERE stage='awarded'").first(),
            db.prepare('SELECT COUNT(*) as cnt FROM vendors').first(),
            db.prepare('SELECT COUNT(*) as cnt FROM proposals').first(),
            db.prepare('SELECT COUNT(*) as cnt FROM email_log').first(),
        ]);
...
exports.apiRouter.get('/rfps/:id/pdf-content', async (c) => {
    ...
    if (!rfp)
        return c.json({ error: 'Not found' }, 404);
    if (!rfp.content)
        return c.json({ error: 'RFP has no generated content yet' }, 400);
    return new Response(rfp.content || '', {
        status: 200,
        headers: {
...
exports.apiRouter.get('/rfps/:id/pdf', async (c) => {
    ...
    if (!rfp.content)
        return c.json({ error: 'RFP has no generated content yet' }, 400);
    const safeRef = (rfp.ref_number || String(id)).replace(/\//g, '_').replace(/[^a-zA-Z0-9_\-]/g, '');
...
exports.apiRouter.post('/rfps/:id/generate', async (c) => {
    ...
    const existingRfp = await c.env.DB.prepare('SELECT * FROM rfps WHERE id=?').bind(id).first().catch(() => null);
    ...
    const sendProgress = async (step, detail) => {
    ...
    const llmCall = async (sp, up, model, maxTok) => {
    ...
    const streamTask = (async () => {
        try {
            ...
            const _LOC_MAP = { AED: 'Dubai / Abu Dhabi, UAE', BHD: 'Manama, Bahrain', SAR: 'Riyadh, Saudi Arabia', QAR: 'Doha, Qatar', KWD: 'Kuwait City, Kuwait', OMR: 'Muscat, Oman', EGP: 'Cairo, Egypt', EUR: 'Western Europe', GBP: 'London, United Kingdom', INR: 'Bangalore, India' };
            ...
            if (existingScoringMatrix) {
                try {
                    ...
                    if (Array.isArray(mx) && mx.length > 0) {
                        scoringMatrixNote = mx.map((r) => `${r.criterion}: ${r.weight}% — ${r.description || ''}`).join('\n');
                    }
                }
                catch { /* ignore */ }
            }
...
Return ONLY the JSON object. No markdown. No explanation.`;
            ...
            let outline = {};
            try {
                outline = JSON.parse(outlineClean);
            }
            catch {
...
${brdDocText && brdDocText.length > 500 ? `BRD:\n${brdDocText.slice(0, 8000)}` : ''}`.trim();
            ...
            const genSection = (sectionKey, sectionSpec, extraInstruction) => {
...
Write the complete Markdown for section "${sectionSpec?.heading || sectionKey}" now. Minimum word count: ${sectionSpec?.min_words || 150}. Output Markdown only — no HTML, no code fences.`;
          ...
          WHERE id=?
        `).bind(body.title, body.category, body.budget, body.deadline, body.scope, body.tech_requirements || '', body.objectives || '', body.background || '', content, rfpFullText, archDocText, brdDocText, id).run();
            }
            const rfp = await c.env.DB.prepare('SELECT * FROM rfps WHERE id=?').bind(id).first().catch(() => null);
        ...
        while ((um = utf16Re.exec(raw)) !== null) {
            try {
                ...
                for (let i = 0; i < s.length; i++)
                    utf16Bytes.push(s.charCodeAt(i));
...
exports.apiRouter.post('/rfps/:id/upload-arch-doc', async (c) => {
    try {
        ...
        const docLabel = (formData.get('doc_label') || '').toLowerCase();
...
exports.apiRouter.post('/rfps/upload-rfp-pdf', async (c) => {
    try {
        ...
        if (!file)
            return c.json({ error: 'No file uploaded' }, 400);
        const rfpCurrency = (formData.get('rfp_currency') || 'USD').toUpperCase();
        const countryOfIssue = (formData.get('country_of_issue') || '').trim();
        const category = (formData.get('category') || 'IT & Digital Transformation');
        const arrayBuffer = await file.arrayBuffer();
        const bytes = new Uint8Array(arrayBuffer);
        const sizeKb = Math.round(bytes.length / 1024);
    ...
    `).bind(refNum, titleFromFilename, category, rfpCurrency, countryOfIssue, r2Key, placeholder, file.name).run();
        const rfpId = r.meta.last_row_id;
        ...
        if (r2Key) {
            ...
            (async () => {
                try {
                    ...
                    await c.env.DB.prepare(`UPDATE rfps SET uploaded_rfp_text=?, rfp_full_text=?, ai_extraction_status='extracting', updated_at=datetime('now') WHERE id=?`).bind(extractedText, extractedText.slice(0, 100000), rfpId).run();
                    // Run OpenAI extraction inline (same as the old callback handler)
                    const { extracted, scoringMatrixJson, requirementGlossaryJson } = await extractRfpFieldsFromOcr(extractedText, c.env, `rfp=${rfpId}`);
...
function cleanOcrText(text) {
    return text
        ...
        if (nonSpace.length > 8 && (nonSpace.match(/\.+/g) || []).join('').length / nonSpace.length > 0.4)
            return false;
...
function extractFocusedSection(ocrText, keywords, maxChars) {
    ...
    for (const kw of keywords) {
        ...
        while (true) {
            ...
            const isTocLine = /\.{6,}/.test(lineText) || /\.{3,}\s*\d+\s*$/.test(lineText.trim());
            if (!isTocLine) {
                if (cleanedAnchor === -1 || idx < cleanedAnchor)
                    cleanedAnchor = idx;
                break;
            }
            searchFrom = idx + 1;
...
async function callSidecarLlmExtract(rfpId, ocrText, callbackUrl, env) {
    const sidecarUrl = env?.PDF_SIDECAR_URL || globalThis.PDF_SIDECAR_URL || '';
    const sidecarSecret = env?.PDF_SIDECAR_SECRET || globalThis.PDF_SIDECAR_SECRET || '';
...
async function extractRfpFieldsFromOcr(ocrText, env, rfpIdLog) {
    const phaseErrors = [];
...
Do NOT output a \`vendor_requirements\` key. Return only the keys listed above.`;
    ...
    let extracted = {};
    let scoringMatrixJson = null;
    let requirementGlossaryJson = null;
    try {
...
async function writeExtractedRfpFields(db, rfpId, ocrText, extracted, scoringMatrixJson, requirementGlossaryJson) {
    ...
    WHERE id = ?
  `).bind(newTitle, newTitle, newCategory, newCategory, extracted.background || '', extracted.background || '', extracted.objectives || '', extracted.objectives || '', extracted.scope || '', extracted.scope || '', extracted.tech_requirements || '', extracted.tech_requirements || '', extracted.budget || '', extracted.budget || '', extracted.deadline || '', extracted.deadline || '', ocrText.slice(0, 100000), scoringMatrixJson, scoringMatrixJson, requirementGlossaryJson, requirementGlossaryJson, rfpId).run();
...
exports.apiRouter.post('/callback/rfps/:rfpId/rfp-upload-ocr-complete', async (c) => {
    ...
    const extractedText = body.text.slice(0, 120000);
    ...
    const db = c.env.DB;
    try {
        const { extracted, scoringMatrixJson, requirementGlossaryJson, phaseErrors } = await extractRfpFieldsFromOcr(extractedText, c.env, `rfp=${rfpId}`);
        const { newTitle } = await writeExtractedRfpFields(db, rfpId, extractedText, extracted, scoringMatrixJson, requirementGlossaryJson);
...
exports.apiRouter.post('/rfps/:id/rerun-ai-extraction', async (c) => {
    ...
    try {
        ...
        const { extracted, scoringMatrixJson, requirementGlossaryJson, phaseErrors } = await extractRfpFieldsFromOcr(ocrText, c.env, `rfp=${rfpId}`);
        ...
        const { newTitle } = await writeExtractedRfpFields(c.env.DB, rfpId, ocrText, extracted, scoringMatrixJson, requirementGlossaryJson);
        await c.env.DB.prepare(`UPDATE rfps SET ai_extraction_status='done', updated_at=datetime('now') WHERE id=?`).bind(rfpId).run();
...
exports.apiRouter.post('/callback/rfps/:rfpId/llm-extract-complete', async (c) => {
    ...
    const rfpRow = await c.env.DB.prepare('SELECT uploaded_rfp_text FROM rfps WHERE id=?').bind(rfpId).first();
    const ocrText = rfpRow?.uploaded_rfp_text || '';
    try {
        const { newTitle } = await writeExtractedRfpFields(c.env.DB, rfpId, ocrText, scalarFields, scoringMatrixJson, requirementGlossaryJson);
...
- Number sequentially: req_1, req_2, req_3, ...`;
        ...
        await c.env.DB.prepare(`UPDATE rfps SET requirement_glossary=?, updated_at=datetime('now') WHERE id=?`).bind(glossaryJson, rfpId).run();
...
exports.apiRouter.post('/rfps/:id/inject-fields', async (c) => {
    ...
    try {
        const body = await c.req.json();
        const rfp = await c.env.DB.prepare('SELECT id FROM rfps WHERE id=?').bind(rfpId).first();
        if (!rfp)
            return c.json({ error: 'RFP not found' }, 404);
        const updates = [];
        const bindings = [];
        ...
        if (body.rfp_full_text !== undefined) {
            updates.push('rfp_full_text=?');
            bindings.push(body.rfp_full_text);
        }
        ...
        if (body.background !== undefined) {
            ...
            bindings.push(body.background);
        }
        if (body.objectives !== undefined) {
            updates.push('objectives=?');
            bindings.push(body.objectives);
        }
        if (body.scope !== undefined) {
            updates.push('scope=?');
            bindings.push(body.scope);
        }
        if (body.tech_requirements !== undefined) {
            updates.push('tech_requirements=?');
            bindings.push(body.tech_requirements);
        }
        if (body.budget !== undefined) {
            updates.push('budget=?');
            bindings.push(body.budget);
        }
...
exports.apiRouter.put('/vendors/:id', async (c) => {
    ...
    try {
        const body = await c.req.json();
        const { contact_email, contact_name, website, hq_city, founded_year, annual_revenue_usd, platforms, certifications, erp_experience, public_sector_refs, specializations } = body;
      ...
      WHERE id=?`)
            .bind(contact_email.trim().toLowerCase(), contact_name?.trim() || null, website?.trim() || null, hq_city?.trim() || null, founded_year || null, annual_revenue_usd?.trim() || null, platforms?.trim() || null, certifications?.trim() || null, erp_experience?.trim() || null, public_sector_refs?.trim() || null, specializations?.trim() || null, id).run();
...
exports.apiRouter.post('/rfps/:id/questions/draft-all', async (c) => {
    ...
    const { results: qs } = await c.env.DB.prepare(`SELECT * FROM questions WHERE (emailed_at IS NULL OR emailed_at='') AND rfp_id=?`).bind(rfpId).all();
    const BATCH_SIZE = 5;
    let manualCount = 0;
...
exports.apiRouter.post('/rfps/:id/questions/publish-all', async (c) => {
    ...
    `).bind(rfpId).all();
        ...
        const anonymizedQuestions = allQuestions.map((q) => ({
            ...q,
...
exports.apiRouter.post('/rfps/:id/emails/send-invitations', async (c) => {
  ...
  `).bind(rfpId).all();
    ...
    const pdfBase64 = body.pdf_base64 || undefined;
    const pdfFilename = body.pdf_filename || undefined;
    const results = [];
    for (const v of shortlisted) {
        ...
        const existing = await c.env.DB.prepare(`SELECT id, status FROM email_log WHERE vendor_id=? AND rfp_id=? AND email_type='invitation'`).bind(v.id, rfpId).first();
...
exports.apiRouter.post('/rfps/:id/emails/check-inbox', async (c) => {
    ...
    const andersen = await c.env.DB.prepare(`SELECT * FROM vendors WHERE contact_email LIKE '%andersenlab.com%' LIMIT 1`).first();
    ...
    `).bind(rfpId, vendorId, andersen.contact_email, `RE: Q&A Questions – ${rfp?.title || 'RFP'} (${newCount} questions)`, `Vendor submitted ${newCount} clarification questions regarding the RFP.`).run();
    }
...
exports.apiRouter.post('/webhook/inbound-email', async (c) => {
    try {
        ...
        if (!apiKey)
            return c.json({ ok: false, error: 'no api key' }, 500);
        // Fetch full email content from Resend API
        const emailRes = await fetch(`https://api.resend.com/emails/receiving/${emailId}`, {
            headers: { 'Authorization': `Bearer ${apiKey}` }
        });
        if (!emailRes.ok)
            return c.json({ ok: false, error: 'failed to fetch email' }, 500);
        const email = await emailRes.json();
        const fromAddress = email.from || payload.data?.from || '';
        ...
        for (const ref of candidateRefs) {
            const match = await db.prepare(`SELECT * FROM rfps WHERE ref_number=? LIMIT 1`).bind(ref).first();
            if (match) {
                rfp = match;
                break;
            }
        ...
        if (!rfp) {
            rfp = await db.prepare(`SELECT * FROM rfps WHERE stage='qa_open' ORDER BY updated_at DESC LIMIT 1`).first()
                ?? await db.prepare(`SELECT * FROM rfps ORDER BY updated_at DESC LIMIT 1`).first();
        }
        if (!rfp)
            return c.json({ ok: true, note: 'no active rfp' });
        const rfpId = rfp.id;
        ...
        if (participantCodeMatch) {
            ...
            vendorRow = await db.prepare(`SELECT * FROM vendors WHERE id=?`).bind(codeVendorId).first();
            if (codeRfpId && codeRfpId !== rfpId) {
                const codeRfp = await db.prepare(`SELECT * FROM rfps WHERE id=?`).bind(codeRfpId).first();
                if (codeRfp) {
                    rfp = codeRfp;
                }
            }
        ...
        if (!vendorRow && fromAddress) {
            vendorRow = await db.prepare(`SELECT * FROM vendors WHERE contact_email=? OR contact_email LIKE ? LIMIT 1`).bind(fromAddress, `%${fromAddress.split('@')[1] || 'NOMATCH'}%`).first();
        }
        const vendorId = vendorRow?.id || null;
        const vendorDisplayName = vendorRow?.name || fromAddress || 'Vendor';
        // ── Attachment detection ─────────────────────────────────────
        const hasAttachment = attachments.length > 0;
        const spreadsheetAttachment = attachments.find((a) => a.filename?.match(/\.(xlsx|xls|csv)$/i) ||
            a.content_type?.includes('spreadsheet') ||
            a.content_type?.includes('excel') ||
            a.content_type?.includes('csv'));
        const pdfAttachment = attachments.find((a) => a.filename?.match(/\.pdf$/i) ||
            ...
            try {
                ...
                if (intentRes.ok) {
                    ...
                    if (raw.startsWith('DECLINE'))
                        llmVerdict = 'DECLINE';
                    else if (raw.startsWith('QUESTIONS'))
                        llmVerdict = 'QUESTIONS';
                    else
                        llmVerdict = 'NEUTRAL';
                }
            }
        ...
        `).bind(rfpId, vendorId, fromAddress, (0, index_1.getActiveProfile)().procurementEmail, `RE: ${subject} — Q&A Period Closed`, `Auto-reply sent: Q&A closed for RFP ${rfp?.ref_number}. Question from ${vendorDisplayName} rejected.`).run();
            ...
            try {
                ...
                if (attachListRes.ok) {
                    ...
                    const allAttachData = attachList.data || [];
                    const attachData = allAttachData.find((a) => a.id === spreadsheetAttachment?.id) || allAttachData[0];
                    if (attachData?.download_url) {
                        const fileRes = await fetch(attachData.download_url);
                        if (fileRes.ok) {
                            const fileBuffer = await fileRes.arrayBuffer();
                            const filename = spreadsheetAttachment?.filename || '';
...
exports.apiRouter.post('/rfps/:id/vendors/:vendorId/reply', async (c) => {
    ...
    try {
        ...
        const rfp = await c.env.DB.prepare('SELECT * FROM rfps WHERE id=?').bind(rfpId).first();
        const vendor = await c.env.DB.prepare('SELECT * FROM vendors WHERE id=?').bind(vendorId).first();
        if (!vendor)
            return c.json({ ok: false, error: 'Vendor not found' }, 404);
        ...
        const rfpVendorRow = await c.env.DB.prepare(`SELECT status FROM rfp_vendors WHERE rfp_id=? AND vendor_id=?`).bind(rfpId, vendorId).first();
...
exports.apiRouter.post('/rfps/:id/proposals/sample', async (c) => {
  ...
  `).bind(rfpId).all();
    const targets = shortlisted.length > 0 ? shortlisted : (await c.env.DB.prepare('SELECT * FROM vendors LIMIT 8').all()).results;
...
exports.apiRouter.post('/rfps/:rfpId/proposals/:proposalId/award', async (c) => {
    ...
    try {
        ...
        const proposal = await db.prepare('SELECT * FROM proposals WHERE id=? AND rfp_id=?').bind(proposalId, rfpId).first();
        if (!proposal)
        ...
        const vendor = await db.prepare('SELECT name, contact_email FROM vendors WHERE id=?').bind(proposal.vendor_id).first();
        const rfp = await db.prepare('SELECT ref_number, title FROM rfps WHERE id=?').bind(rfpId).first();
        await db.prepare(`
      INSERT INTO email_log (rfp_id, vendor_id, recipient, subject, body, email_type, status, created_at)
...
function extractProposalText(proposal) {
    ...
    if (proposal.executive_summary)
        parts.push(proposal.executive_summary);
    if (proposal.key_strengths)
        parts.push(proposal.key_strengths);
    try {
        const atts = JSON.parse(proposal.proposal_attachments || '[]');
        for (const a of atts) {
            if (a.extracted_text)
                parts.push(String(a.extracted_text));
        }
    }
    catch (_) { }
    return parts.join('\n\n');
...
function topChunks(reqText, chunks, n = 2) {
    return chunks
...
function checkCompliance(reqText, proposalText) {
...
function extractBudget(text) {
    ...
    while ((m = totalRe.exec(text)) !== null) {
        ...
        if (amt > 0)
            return { amount: amt, currency: cur, confidence: 1.0 };
    }
    ...
    while ((m = anyRe.exec(text)) !== null) {
        const amt = parseFloat(m[2].replace(/,/g, ''));
        if (amt > best.amount && amt < 1e10) {
            best = { amount: amt, currency: m[1] || 'USD' };
        }
    }
    if (best.amount > 1000)
        return { amount: best.amount, currency: best.currency, confidence: 0.3 };
...
function reorderProposalSections(text) {
    if (!text)
        return text;
    ...
    if (parts.length <= 1)
        return text; // single file or legacy text — nothing to reorder
    const labelPriority = (section) => {
...
function extractFileLabels(text) {
...
async function evaluateProposal(proposal, rfp, env) {
        ...
        if (!hasTechnical && !hasCommercial) {
            try {
                ...
                const contextNote = hasSupporting
                    ...
                    : '';
                const raw = await callLLM('You are a document classifier for a procurement system. Answer only with valid JSON.', `Classify the following document excerpt. Is this content part of a vendor's proposal submission in response to an RFP / tender?
...
Respond ONLY with JSON: {"is_proposal": true|false, "reason": "<one sentence, max 15 words>"}`, env, 'gpt-5.4-mini', 80);
    ...
    let budget = { amount: null, currency: 'USD', confidence: 0.0 };
    let duration = proposal.proposed_duration || null;
    try {
        // Try to isolate the labelled commercial section
        let budgetInputText = proposalText;
        const commercialSectionMatch = proposalText.match(/={3} FILE:[^\n]*\[label:\s*commercial[^\]]*\][^\n]*\n([\s\S]*?)(?:={3} FILE:|$)/i);
        ...
        if (budgetResult.duration)
            duration = budgetResult.duration;
    ...
    try {
        const matrixArr = JSON.parse(rfp.scoring_matrix || '[]');
        if (matrixArr.length > 0) {
            const commercialCrit = matrixArr.find((c) => /commercial|cost competitiveness|price/i.test(c.criterion || c.name || ''));
            if (commercialCrit) {
                commercialWeight = Number(commercialCrit.weight) || 10;
            }
            const allWeights = matrixArr.reduce((sum, c) => sum + (Number(c.weight) || 0), 0);
            if (allWeights > 0)
                technicalTotal = allWeights - commercialWeight;
        }
    }
    catch (_) { }
    ...
    const rfpFullText = rfpFullTextRaw.length > RFP_TEXT_LIMIT
        ...
        : rfpFullTextRaw;
    // ── Build proposal text for LLM — full technical + commercial, supporting optional ──
    // v101 rules:
    //   • Technical and commercial files are sent IN FULL (no per-file char cap).
    //     These contain the core proposal content the eval LLM must score.
    //   • Supporting documents (CVs, certs, company profiles) are OPTIONAL.
    //     If present, include a short excerpt (first 1500 chars) as supplementary
    //     context. Their absence must NOT penalise the vendor.
    //   • Total cap: 60k chars before the LLM call (gpt-5.4-mini 128k context window;
    //     at ~4 chars/token that's ~15k tokens — well within limits).
    //   • If no file sections exist (legacy / single-file), pass the full text up to 60k.
    const EVAL_TOTAL_LIMIT = 60_000; // chars total sent to eval LLM
    const proposalTextForLLM = (() => {
        const sectionRe = /(?=={3} FILE:)/g;
        const rawSections = proposalText.split(sectionRe).filter(s => s.trim().length > 50);
        if (rawSections.length <= 1) {
            // Single file or legacy text — pass full text up to limit
            const out = proposalText.slice(0, EVAL_TOTAL_LIMIT);
            console.log(`[eval-v101] single/legacy text: ${out.length} chars`);
            return out;
        }
        const getLabelType = (s) => {
        ...
        const byType = { technical: [], commercial: [], supporting: [], other: [] };
        for (const sec of rawSections)
            byType[getLabelType(sec)].push(sec);
        const parts = [];
        let totalChars = 0;
        // Technical: full content — primary scoring basis
        for (const sec of byType.technical) {
            parts.push(sec);
            totalChars += sec.length;
        }
        // Commercial: full content — pricing, timeline
        for (const sec of byType.commercial) {
            parts.push(sec);
            totalChars += sec.length;
        }
        // Supporting: OPTIONAL supplementary context — first 1500 chars each, max 5 files.
        // Only included if total budget permits. Absence does NOT penalise the vendor.
        const supportingSections = [...byType.supporting, ...byType.other].slice(0, 5);
        for (const sec of supportingSections) {
            if (totalChars >= EVAL_TOTAL_LIMIT)
                break;
            const fnMatch = sec.match(/=== FILE:\s*([^\[]+)/);
            const fname = fnMatch ? fnMatch[1].trim() : 'supporting document';
            const body = sec.replace(/^={3} FILE:[^\n]+\n/, '').trim();
            const excerpt = body.slice(0, 1_500);
            const block = `=== SUPPORTING DOCUMENT (optional context): ${fname} ===\n${excerpt}${body.length > 1_500 ? '\n[... truncated — supplementary only ...]' : ''}`;
            parts.push(block);
            totalChars += block.length;
        }
        const assembled = parts.join('\n\n').slice(0, EVAL_TOTAL_LIMIT);
        console.log(`[eval-v101] assembled ${assembled.length} chars from ${rawSections.length} file sections (tech=${byType.technical.length} comm=${byType.commercial.length} supp=${supportingSections.length})`);
        return assembled;
    })();
    ...
    const evalSystemPrompt = `You are a procurement evaluation expert. You score vendor proposals against RFP criteria based strictly on the content provided.

The vendor submission may contain multiple labelled file sections:
  • [label: technical]  — Technical proposal: methodology, architecture, solution approach, team
  • [label: commercial] — Commercial proposal: pricing, timeline, payment terms
  • SUPPORTING DOCUMENT sections — Optional supplementary materials (CVs, certifications, company profile, references). These are OPTIONAL extras. Their presence improves the submission; their absence is NOT a disqualifying gap unless the RFP explicitly listed them as a mandatory deliverable scored in its own criterion.

Score only against what the RFP criteria state. Do not penalise for absent supporting documents unless those documents are an explicitly scored criterion in the Scoring Matrix.`;
    const evalUserPrompt = `Evaluate the vendor's proposal against the RFP criteria below. Exclude Commercial/Cost scoring (handled separately); score only the technical criteria.

Scoring scale (applied per criterion, out of its weight):
• 90–100% = Excellent: fully meets and exceeds with clear evidence
• 70–89%  = Good: meets core requirements, minor gaps
• 50–69%  = Partial: meets some but lacks important elements
• < 50%   = Poor: fails to address criterion or critical non-compliance

Rules:
1. Base every score only on content visible in the documents below.
2. For mandatory requirements: apply a severe penalty (≤33% of criterion weight) ONLY if a requirement is explicitly listed as mandatory in the RFP AND is entirely absent from the technical AND supporting sections.
3. Supporting documents (CVs, certs, profiles) are supplementary — note them as a strength if present; do NOT list them as a weakness or apply penalties if absent unless they are an explicit scored criterion.
4. Justify each score with specific evidence from the proposal text (quote or reference the section).
  ...
  "scores": [
    {
      ...
      "weight": 30,
      "score_achieved": 28,
      "justification": "Specific evidence from proposal..."
    }
  ],
  "total_score": 65.0,
...
Now return the JSON evaluation object.`;
    ...
    try {
        ...
        totalScore = typeof parsed.total_score === 'number'
            ...
            : scores.reduce((sum, s) => sum + (Number(s.score_achieved) || 0), 0);
        // scoring_breakdown — direct pass-through of LLM criterion scores
        scoringBreakdown = scores.map((s) => ({
        ...
        mandatoryFailed = scores
            .filter((s) => Number(s.weight) >= 10 && (Number(s.score_achieved) / Math.max(1, Number(s.weight))) < 0.34)
            .map((s) => `${s.criterion} (${s.score_achieved}/${s.weight})`);
        ...
        const scopePct = typeof parsed.scope_alignment_pct === 'number'
        ...
        if (scopePct < 60) {
            ...
            totalScore = 0;
            scoringBreakdown = scoringBreakdown.map((row) => ({ ...row, score_achieved: 0, achieved_pct: 0 }));
            complianceBreakdown = complianceBreakdown.map((row) => ({ ...row, compliance_met: false, ai_score: 0 }));
            strengths = [];
    ...
    //   4. 'USD' hard default
    function detectRfpCurrency(budgetField, text, issuerCurrency, rfpStoredCurrency) {
        ...
        if (bfStr) {
            for (const code of ALL_CODES) {
                if (new RegExp(`\\b${code}\\b`, 'i').test(bfStr))
                    return code;
            }
            // Symbol scan on budget field
            if (/د\.إ|AED/i.test(bfStr))
                return 'AED';
            if (/ر\.س|SAR/i.test(bfStr))
                return 'SAR';
            if (/ر\.ق|QAR/i.test(bfStr))
                return 'QAR';
            if (/€/.test(bfStr))
                return 'EUR';
            if (/£/.test(bfStr))
        ...
        let match;
        const zones = [];
        while ((match = nearBudgetRe.exec(budgetZone)) !== null)
            zones.push(match[0]);
        const zoneText = zones.join(' ');
        if (zoneText) {
            for (const code of ALL_CODES) {
                if (new RegExp(`\\b${code}\\b`, 'i').test(zoneText))
                    return code;
            }
            if (/د\.إ/.test(zoneText))
                return 'AED';
            if (/€/.test(zoneText))
                return 'EUR';
            if (/£/.test(zoneText))
    ...
    let commercialScoreActual = null;
    ...
    if (budget.amount && commercialWeight > 0) {
        ...
        let rfpBudgetCeiling = null;
        if (rfp.budget) {
            const rfpBudgetNum = parseFloat(String(rfp.budget).replace(/[^\d.]/g, ''));
            if (rfpBudgetNum > 0 && rfpBudgetNum < 1e10)
                rfpBudgetCeiling = rfpBudgetNum;
        }
        ...
        if (!rfpBudgetCeiling && rfpFullText) {
            ...
            for (const pat of budgetPatterns) {
                ...
                if (m) {
                    ...
                    if (val > 10000 && val < 1e10) {
                        rfpBudgetCeiling = val;
                        break;
                    }
                }
            }
...
exports.apiRouter.post('/callback/proposals/:proposalId/file-ocr-complete', async (c) => {
    ...
    try {
        ...
        const existing = await db.prepare(`SELECT proposal_full_text FROM proposals WHERE id=?`).bind(proposalId).first();
        const currentText = existing?.proposal_full_text || '';
      ...
      WHERE id=?
    `).bind(proposalId).run();
        const fresh = await db.prepare(`SELECT ocr_pending_files, proposal_full_text FROM proposals WHERE id=?`).bind(proposalId).first();
...
exports.apiRouter.post('/rfps/:id/proposals/evaluate-all', async (c) => {
    ...
    const db = c.env.DB;
    try {
        const rfp = await db.prepare('SELECT * FROM rfps WHERE id=?').bind(rfpId).first();
        if (!rfp)
            return c.json({ error: 'RFP not found' }, 404);
    ...
    `).bind(rfpId).all();
        if (!proposals.length)
            return c.json({ ok: true, evaluated: 0, message: 'No proposals to evaluate' });
        ...
        const readyProposals = proposals.filter((p) => isReadyP(p));
        const results = [];
        ...
        for (const proposal of readyProposals) {
          ...
          WHERE id=?
        `).bind(JSON.stringify(evalData), evalData.total_score, evalData.recommendation, evalData.validation_status, evalData.compliance_score, evalData.quality_score, evalData.commercial_score, proposal.id).run();
        ...
        return c.json({ ok: true, evaluated: results.filter((r) => !r.skipped).length, skipped: notReady.length, results });
    }
    catch (e) {
        return c.json({ ok: false, error: e?.message }, 500);
    }
...
exports.apiRouter.post('/callback/proposals/:proposalId/ocr-complete', async (c) => {
    ...
    try {
        ...
        const ocrText = (body.text || '').slice(0, 50000);
        ...
        // 2. Load proposal + RFP
        const proposal = await db.prepare(`SELECT p.*, v.name as vendor_name FROM proposals p LEFT JOIN vendors v ON p.vendor_id=v.id WHERE p.id=?`).bind(proposalId).first();
        if (!proposal)
            return c.json({ error: 'Proposal not found' }, 404);
        const rfpRow = rfpId
            ? await db.prepare('SELECT * FROM rfps WHERE id=?').bind(rfpId).first()
            : await db.prepare('SELECT * FROM rfps WHERE id=?').bind(proposal.rfp_id).first();
        if (!rfpRow)
            return c.json({ error: 'RFP not found' }, 404);
        ...
        const freshProposal = await db.prepare(`SELECT p.*, v.name as vendor_name FROM proposals p LEFT JOIN vendors v ON p.vendor_id=v.id WHERE p.id=?`).bind(proposalId).first();
      ...
      UPDATE proposals SET
        ...
        ocr_job_status='done', status='evaluated', updated_at=datetime('now')
      WHERE id=?
    `).bind(JSON.stringify(evalData), totalScore, recommendation, evalData.validation_status ?? 'EVALUATED', Math.round(evalData.compliance_score ?? totalScore), Math.round(evalData.quality_score ?? totalScore), proposalId).run();
...
exports.apiRouter.post('/rfps/:rfpId/proposals/:proposalId/evaluate-budget', async (c) => {
    ...
    `).bind(proposalId, rfpId).first();
        ...
        const fullText = extractProposalText(proposal);
        ...
        const commercialMatch = fullText.match(/={3} FILE:[^\n]*\[label:\s*commercial[^\]]*\][^\n]*\n([\s\S]*?)(?:={3} FILE:|$)/i);
...
async function runBudgetLLM(proposalText, proposal, db, env) {
...
${proposalText.slice(0, 30_000)}${proposalText.length > 30_000 ? '\n\n[... text truncated at 30k chars; price tables are typically in the first section ...]' : ''}`;
    ...
    if (!jsonMatch)
        return { budget_amount: null, budget_currency: 'USD', budget_confidence: 0, duration: null, missing_info: ['LLM returned no JSON'] };
    let parsed;
    try {
    ...
    const duration = (llmMonths >= dbMonths) ? (llmDuration || dbDuration) : (dbDuration || llmDuration);
    ...
    `).bind(budgetAmount, budgetCurrency, duration, proposal.id).run();
        try {
            let evalData = {};
            try {
                evalData = JSON.parse(proposal.evaluation_data || '{}');
            }
            catch (_) { }
    ...
    if (lineItems.length) {
        ...
        const excluded = lineItems.filter((l) => !l.included);
        console.log(`[budget-llm] line_items: ${included.length} included, ${excluded.length} excluded`);
        included.forEach((l) => console.log(`  [+] ${l.name}: ${l.currency} ${l.amount} — ${l.reason}`));
        excluded.forEach((l) => console.log(`  [-] ${l.name}: ${l.currency} ${l.amount} — ${l.reason}`));
...
exports.apiRouter.get('/rfps/:rfpId/proposals/:proposalId/evaluation', async (c) => {
    ...
    let evalData = null;
    try {
        evalData = proposal.evaluation_data ? JSON.parse(proposal.evaluation_data) : null;
    }
    catch (_) { }
    return c.json({
        proposal_id: proposal.id,
...
exports.apiRouter.post('/rfps/:rfpId/market-benchmark', async (c) => {
    ...
    try {
        ...
        // ── Use structured RFP fields, NOT raw rfp_full_text ─────────────────────────
        // rfp_full_text is the raw OCR of the uploaded PDF and can contain noisy or
        // domain-specific boilerplate that causes the LLM to hallucinate irrelevant roles
        // (e.g. "Digital Banking Product Consultant" for a data analytics RFP).
        // The structured fields (scope, objectives, category, tech_requirements) were
        // extracted and summarised by the AI and accurately represent what was requested.
        const category = rfp.category || 'IT / Software Development';
        const title = rfp.title || 'Unnamed Project';
        const scope = (rfp.scope || '').slice(0, 3000);
        const objectives = (rfp.objectives || '').slice(0, 1500);
        const techReqs = (rfp.tech_requirements || '').slice(0, 1500);
        const background = (rfp.background || '').slice(0, 1000);
        // Only use rfp_full_text as a last resort if no structured fields exist
        const hasStructured = scope.length > 50 || objectives.length > 50;
        const rfpContextText = hasStructured
            ? [
                scope ? `Scope of Work:\n${scope}` : '',
            ...
            : (rfp.rfp_full_text || rfp.content || '').slice(0, 8000);
        if (!rfpContextText.trim()) {
            return c.json({ error: 'RFP content not yet generated — please generate the RFP document first.' }, 400);
        }
        const archText = (rfp.arch_doc_text || '').slice(0, 3000);
        const brdText = (rfp.brd_doc_text || '').slice(0, 3000);
        const systemPrompt = `You are a senior IT project estimator. Your job is to produce a realistic WBS and market-rate cost estimate STRICTLY based on the project scope provided. You must NOT add roles, workstreams, or technologies that are not mentioned or clearly implied by the scope. Every role in team_composition must be directly justified by the stated scope.`;
        const userPrompt = `Estimate the market-average implementation cost for the following project, issued by ${issuerName} in ${issuerLoc}.
Use ${issuerLoc} market day rates. All monetary values in ${benchmarkCurrency}.
...
--- PROJECT SCOPE (use ONLY this to derive WBS phases and team roles) ---
${rfpContextText}
...
CRITICAL RULES:
1. WBS phases must map 1:1 to the scope workstreams above. Do not invent phases not present in the scope.
2. Team roles must be generic delivery roles (e.g. Project Manager, Solution Architect, Backend Developer, QA Engineer, Business Analyst). Do NOT use client-industry-specific titles (e.g. "Banking Consultant", "Retail Specialist") unless the scope explicitly states a banking or retail context.
3. Use ${issuerLoc} market day rates for the roles appropriate to this type of project (${category}).
4. Produce min/mid/max total cost range in ${benchmarkCurrency}.
...
Return ONLY valid JSON — no markdown, no commentary:
{
  ...
  "total_mid": 1850000,
  "confidence": "medium",
  "confidence_rationale": "Brief rationale based on scope complexity and regional rates.",
  "wbs": [
    {
  ...
  "team_composition": [
    {
      ...
      "seniority": "Senior",
      "hourly_rate": 400,
      "notes": "Delivery governance and stakeholder reporting"
    },
    {
      ...
      "seniority": "Principal",
      "hourly_rate": 550,
      "notes": "Technical architecture and integration design"
    }
  ],
  "assumptions": ["Day rates reflect mid-market ${issuerLoc} market for ${category} projects", "Excludes hardware, licences, and cloud infrastructure unless in scope"]
}`;
        ...
        const jsonMatch = clean.match(/\{[\s\S]*\}/);
        if (!jsonMatch)
            return c.json({ error: 'LLM returned no JSON for market benchmark' }, 500);
        let parsed;
        try {
            parsed = JSON.parse(jsonMatch[0]);
        }
...
exports.apiRouter.post('/rfps/:rfpId/proposals/:proposalId/manual-override', async (c) => {
    ...
    try {
        const body = await c.req.json();
        const proposal = await db.prepare('SELECT * FROM proposals WHERE id=?').bind(proposalId).first();
        if (!proposal)
            return c.json({ error: 'Not found' }, 404);
        let evalData = {};
        try {
        ...
        if (body.manual_budget) {
            const rfp = await db.prepare('SELECT budget FROM rfps WHERE id=?').bind(c.req.param('rfpId')).first();
            ...
            if (evalData.mandatory_failed?.length > 0)
                evalData.recommendation = 'NOT RECOMMENDED';
            else if (evalData.total_score >= 80)
                evalData.recommendation = 'RECOMMENDED';
            else if (evalData.total_score >= 60)
                evalData.recommendation = 'CONDITIONAL';
            else
                evalData.recommendation = 'NOT RECOMMENDED';
        }
        await db.prepare(`
    ...
    `).bind(JSON.stringify(evalData), evalData.total_score, evalData.recommendation, evalData.validation_status, evalData.commercial_score, proposalId).run();
        return c.json({ ok: true, ...evalData });
    }
    catch (e) {
        return c.json({ ok: false, error: e?.message }, 500);
...
exports.apiRouter.post('/rfps/:id/ingest', async (c) => {
    ...
    try {
        ...
        const raw = await callLLM('You are a procurement analyst. Extract structured requirements from an RFP document.', `Extract all vendor requirements from this RFP text. For each requirement, determine if it is mandatory (contains "must", "shall", "required", "mandatory"). Return a JSON array of objects: [{"id":"req_1","text":"...","mandatory":true/false},...]. Extract up to 20 requirements. Return ONLY the JSON array.\n\nRFP TEXT:\n${rfpText}`, c.env, 'gpt-5.4-mini', 2000);
        let glossary = [];
        try {
            const match = raw.match(/\[[\s\S]*\]/);
            if (match)
                glossary = JSON.parse(match[0]);
        }
        catch (_) { }
        if (!glossary.length) {
...
exports.apiRouter.get('/submit/:rfpId', async (c) => {
    ...
    if (closedStages.includes(rfp.stage))
        return c.json({ error: 'This RFP is no longer accepting submissions.' }, 403);
    ...
    if (vendorCode) {
        ...
        if (codeMatch && Number(codeMatch[1]) === Number(rfpId)) {
            ...
            const rv = await c.env.DB.prepare(`SELECT status FROM rfp_vendors WHERE rfp_id=? AND vendor_id=?`).bind(rfpId, vId).first();
...
async function resolveVendorForSubmit(db, rfpId, vendorCode) {
    const rfp = await db.prepare('SELECT id, stage FROM rfps WHERE id=?').bind(rfpId).first();
    if (!rfp)
        return { error: 'RFP not found', status: 404 };
    if (['awarded', 'archived'].includes(rfp.stage))
        return { error: 'This RFP is no longer accepting submissions.', status: 403 };
    ...
    const v = await db.prepare('SELECT id, name FROM vendors WHERE id=?').bind(vendorId).first();
    if (!v)
        return { error: 'Invalid participant code.', status: 400 };
    const rv = await db.prepare('SELECT status FROM rfp_vendors WHERE rfp_id=? AND vendor_id=?').bind(rfpId, vendorId).first();
    if (rv?.status === 'declined')
        return { error: 'declined', status: 403 };
...
exports.apiRouter.post('/submit/:rfpId/presign', async (c) => {
    ...
    if ('error' in vendor)
        return c.json({ error: vendor.error, ...(vendor.error === 'declined' && { declined: true }) }, vendor.status);
    const { vendorId, vendorName } = vendor;
    const files = body.files || [];
    if (!files.length)
        return c.json({ error: 'No files specified.' }, 400);
    if (files.length > 10)
        return c.json({ error: 'Maximum 10 files per submission.' }, 400);
...
exports.apiRouter.post('/submit/:rfpId/finalize', async (c) => {
    ...
    if ('error' in vendor)
        return c.json({ error: vendor.error, ...(vendor.error === 'declined' && { declined: true }) }, vendor.status);
    const { vendorId, vendorName } = vendor;
    const pendingAttachments = body.attachments || [];
    if (!pendingAttachments.length)
        return c.json({ error: 'No attachments provided.' }, 400);
    const bucket = c.env.PROPOSALS_BUCKET;
    if (!bucket)
        return c.json({ error: 'Storage not configured.' }, 500);
    ...
    if (!storedAttachments.length)
        return c.json({ error: 'No files stored successfully.' }, 500);
    ...
    const existing = await c.env.DB.prepare('SELECT id FROM proposals WHERE rfp_id=? AND vendor_id=? ORDER BY id DESC LIMIT 1').bind(rfpId, vendorId).first();
    let proposalId;
    if (existing) {
        await c.env.DB.prepare(`
    ...
    `).bind(rfpId, vendorId, coverLetter || null, proposalAttachmentsJson, pdfUrl, pdfFilename).run();
        proposalId = ins.meta.last_row_id;
    }
    ...
    (async () => {
        for (const att of storedAttachments) {
            ...
            try {
                ...
                const existing2 = await c.env.DB.prepare(`SELECT proposal_full_text FROM proposals WHERE id=?`).bind(proposalId).first();
                ...
                const fresh2 = await c.env.DB.prepare(`SELECT ocr_pending_files, proposal_full_text FROM proposals WHERE id=?`).bind(proposalId).first();
...
exports.apiRouter.post('/submit/:rfpId', async (c) => {
    ...
    try {
        const formData = await c.req.formData();
        const vendorCode = (formData.get('vendor_code') || '').trim().toUpperCase();
        const coverLetter = (formData.get('cover_letter') || '').trim();
        const fileLabels = {};
        try {
            const labelsJson = formData.get('file_labels');
            if (labelsJson)
                Object.assign(fileLabels, JSON.parse(labelsJson));
        }
        catch (_) { }
        ...
        if (vendorCode) {
            ...
            if (codeMatch && Number(codeMatch[1]) === Number(rfpId)) {
                ...
                const v = await c.env.DB.prepare('SELECT id, name FROM vendors WHERE id=?').bind(vendorId).first();
                if (v) {
                    vendorId = v.id;
                    vendorName = v.name;
                }
        ...
        const rvStatus = await c.env.DB.prepare(`SELECT status FROM rfp_vendors WHERE rfp_id=? AND vendor_id=?`).bind(rfpId, vendorId).first();
        ...
        const genericFiles = formData.getAll('files');
        for (const gf of genericFiles) {
            if (gf instanceof File)
                files.push(gf);
        }
        if (files.length === 0)
            return c.json({ error: 'Please attach at least one proposal document.' }, 400);
        const bucket = c.env.PROPOSALS_BUCKET;
        const storedAttachments = [];
        ...
        (async () => {
            for (const att of storedAttachments) {
                ...
                try {
                    ...
                    const ex = await c.env.DB.prepare(`SELECT proposal_full_text FROM proposals WHERE id=?`).bind(proposalId).first();
                    const merged = ((ex?.proposal_full_text || '') + '\n\n' + fileText).slice(0, 200000).trim();
                    await c.env.DB.prepare(`UPDATE proposals SET proposal_full_text=?, updated_at=datetime('now') WHERE id=?`).bind(merged, proposalId).run();
                    await c.env.DB.prepare(`UPDATE proposals SET ocr_pending_files=MAX(0,COALESCE(ocr_pending_files,1)-1), updated_at=datetime('now') WHERE id=?`).bind(proposalId).run();
                    const fresh3 = await c.env.DB.prepare(`SELECT ocr_pending_files, proposal_full_text FROM proposals WHERE id=?`).bind(proposalId).first();
...
async function generateRFPWithLLM(data, archDocText, brdDocText, env, scoringMatrixJson, settings) {
...
async function draftAnswerLLM(question, rfp, env) {
...
// ============================================================
function detectAttachmentLabel(filename) {
...
// ============================================================
function generateQAExcel(questions) {
    ...
    const getSSIdx = (s) => {
        if (!ssMap.has(s)) {
            ssMap.set(s, sharedStrings.length);
    ...
    const files = {
        ...
        '_rels/.rels': encodeUtf8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>`
            + `<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">`
...
function generateRfpPdf(rfp) {
    ...
    function htmlToLines(html) {
        ...
        while (pos < htmlStr.length) {
            ...
            if (lt > pos)
                tokens.push({ type: 'text', text: htmlStr.slice(pos, lt) });
    ...
    let stream = '';
    let pageNum = 1;
    function charsPerWidth(size, w) {
        return Math.floor(w / (size * 0.52));
    }
    function wordWrap(text, size, maxWidth, indent) {
            ...
            if (w.includes('\n')) {
                ...
                for (let pi = 0; pi < parts.length; pi++) {
                    if (pi > 0) {
                        ...
                        line = line ? line + ' ' + parts[pi] : parts[pi];
                }
                continue;
            }
    ...
    if (refNum)
        stream += `(Reference: ${escPdfString(refNum)}) Tj\n0 -13 Td\n`;
    if (category)
        stream += `(Category: ${escPdfString(category)}) Tj\n0 -13 Td\n`;
    if (deadline)
        stream += `(Submission Deadline: ${escPdfString(deadline)}) Tj\n0 -13 Td\n`;
    stream += `ET\n`;
    // Large title
...
// ============================================================
function computeVendorScore(v, rfp) {
    ...
    if (rfpNeedsOracle) {
        ...
        if (specs.includes('sap'))
            score += 35;
        else if (specs.includes('erp'))
            score += 10;
    }
    else if (rfpNeedsData) {
        if (specs.includes('data') || specs.includes('analytics') || specs.includes('bi') || exp.includes('data'))
            score += 25;
        else if (specs.includes('erp'))
            score += 5;
    }
    else if (rfpNeedsCRM) {
        if (specs.includes('crm') || specs.includes('salesforce') || specs.includes('dynamics'))
            score += 30;
        else if (specs.includes('erp'))
            score += 10;
    }
    else if (rfpNeedsERP) {
        if (specs.includes('erp'))
            score += 20;
        else
            score += 5;
    }
    ...
    if (certs.includes('iso 27001'))
        score += 5;
    if (certs.includes('iso 9001'))
        score += 3;
    if (rfpNeedsOracle && (certs.includes('oracle') || certs.includes('ocp') || certs.includes('oce')))
        score += 10;
    if (rfpNeedsSAP && certs.includes('sap'))
        score += 10;
    // Company size
    if (v.size === 'Large')
        score += 5;
...
function buildFitRationale(v, score) {
    const grade = score >= 80 ? 'Excellent fit' : score >= 65 ? 'Good fit' : score >= 50 ? 'Moderate fit' : 'Poor fit';
    const reasons = [];
    const specs = (v.specializations || '').toLowerCase();
    const exp = (v.erp_experience || '').toLowerCase();
    if (specs.includes('oracle'))
        reasons.push('Oracle expertise');
    if (specs.includes('sap'))
        reasons.push('SAP expertise');
    if (specs.includes('data') || specs.includes('analytics'))
        reasons.push('Data/Analytics expertise');
    if (exp.includes('government'))
        reasons.push('Government sector experience');
    if (v.size === 'Large')
...
// ============================================================
function buildParticipantCode(rfpId, vendorId) {
    return `RFP-${rfpId}-V${vendorId}`;
}
function buildInvitationEmailText(v, rfp, qDeadline, sDeadline, notes, baseUrl) {
...
async function sendRealEmail(to, subject, bodyText, rfp, env, pdfBase64, pdfFilenameHint) {
    const toAddr = (to || '').toLowerCase().trim();
    // No domain restriction — send to any valid vendor email address.
    const RESEND_API_KEY = env?.RESEND_API_KEY || globalThis.RESEND_API_KEY || '';
    if (!RESEND_API_KEY) {
...
async function readVendorEmailReplies(rfp) {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (!q) {
                ...
                for (const v of Object.values(row)) {
                    if (String(v).length > longest.length)
                        longest = String(v);
                }
                q = longest.trim();
            }
            if (q.length < 10)
                continue;
            if (/^(question|ref|section|no\.|#|item)/i.test(q))
                continue;
            if (!q.endsWith('?'))
                q += '?';
            questions.push(q);
...
function parseCsvQuestions(text) {
    ...
    for (const line of lines) {
        ...
        if (q.length < 15)
            continue;
        if (/^(question|no\.|ref\.?|#|item|sr\.?|section)/i.test(q))
            continue;
        if (!q.endsWith('?'))
            q += '?';
        questions.push(q);
...
function parseQuestionsFromBody(text) {
    if (!text)
        return [];
    const lines = text.split(/\r?\n/).map(l => l.trim()).filter(l => l.length > 0);
    const questions = [];
...
// ============================================================
function buildVendorProposal(v, isAndersen, isEPAM) {
    if (isAndersen) {
        return {
...
exports.apiRouter.get('/vendors/:id/history', async (c) => {
    try {
       ...
       WHERE rv.vendor_id=? ORDER BY r.created_at DESC LIMIT 50`).bind(id).all();
...
exports.apiRouter.post('/submit/:rfpId/confirmation', async (c) => {
    try {
        ...
        const rfp = await c.env.DB.prepare('SELECT title, ref_number FROM rfps WHERE id=?').bind(rfpId).first();
        if (!rfp)
            return c.json({ error: 'RFP not found' }, 404);
