...
apiRouter.post('/rfps', async (c) => {
  try {
    ...
    const refNum = getActiveProfile().rfpRefPrefix + new Date().getFullYear() + '/' + String(Math.floor(Math.random()*9000)+1000)
...
apiRouter.post('/rfps/:id/generate', async (c) => {
  ...
  const streamTask = (async () => {
    try {
      ...
      const procEmail  = settings?.procurement_email || getActiveProfile().procurementEmail
...
PROJECT DATA:
RFP Reference: ${body.ref_number || getActiveProfile().rfpRefPrefix + new Date().getFullYear() + '/TBD'}
...
${getActiveProfile().orgName}${rfp?.country_of_issue ? ', ' + rfp.country_of_issue : ''}
${getActiveProfile().procurementEmail}
...
──────────────────────────────────────────────`
      ...
      if (resendKey) {
        try {
          const emailPayload = {
            from: `${getActiveProfile().procurementEmailLabel} <${getActiveProfile().procurementEmail}>`,
            to: [vendor.contact_email],
            subject: `Q&A Consolidated Response – ${rfp?.title || `${getActiveProfile().orgName} RFP`} (Ref: ${rfp?.ref_number || ''})`,
            text: emailText,
          ...
          if (sendRes.ok) {
            ...
            `).bind(rfpId, vendor.id, vendor.contact_email,
              `Q&A Consolidated Response – ${rfp?.title || `${getActiveProfile().orgName} RFP`} (Ref: ${rfp?.ref_number || ''})`,
              emailText).run()
          } else {
  ...
  for (const v of shortlisted) {
    ...
    if (isAndersenVendor) {
      ...
      const result = await sendRealEmail(
        v.contact_email,
        `Invitation to Tender – ${rfp?.title || `${getActiveProfile().orgName} RFP`} (Ref: ${rfp?.ref_number || ''})`,
        emailBody,
        rfp,
        c.env,
        pdfBase64,
    ...
    if (emailCategory === 'decline') {
      ...
      if (qaClosedStages.includes(rfp.stage)) {
        ...
        if (resendKey && fromAddress && fromAddress.includes('@')) {
          const rfpTitle = rfp?.title || `${getActiveProfile().orgName} RFP`
...
We regret to inform you that the Q&A period for this Request for Proposal has now closed. ${getActiveProfile().orgName} is no longer able to accept or process clarification questions for this tender.

All vendors have been provided with a consolidated Q&A response document containing answers to all submitted questions. If you have not received this document, please contact ${getActiveProfile().procurementEmail} referencing the RFP above.
...
${getActiveProfile().procurementEmail}`
        ...
        `).bind(rfpId, vendorId, fromAddress, getActiveProfile().procurementEmail,
...
apiRouter.post('/rfps/:id/vendors/:vendorId/reply', async (c) => {
  ...
  try {
    ...
    const replySubject = subject || `RE: Invitation to Tender – ${rfp?.title || `${getActiveProfile().orgName} RFP`} (Ref: ${rfp?.ref_number || ''})`
...
apiRouter.post('/rfps/:rfpId/proposals/:proposalId/award', async (c) => {
  ...
  try {
    ...
    `).bind(
      ...
      `Contract Award Notification – ${rfp?.title || `${getActiveProfile().orgName} RFP`} (Ref: ${rfp?.ref_number || ''})`,
...
Respond ONLY with JSON: {"is_proposal": true|false, "reason": "<one sentence, max 15 words>"}`,
  ...
  const proposalTextForLLM = (() => {
    ...
    for (const sec of supportingSections) {
      if (totalChars >= EVAL_TOTAL_LIMIT) break
...
PROJECT DETAILS
${'='.repeat(60)}
RFP Reference:          ${data.ref_number || getActiveProfile().rfpRefPrefix + new Date().getFullYear() + '/TBD'}
...
async function draftAnswerLLM(question: string, rfp: any, env: any): Promise<{ answer: string, needsManual: boolean }> {
  ...
  const _qProfile = getActiveProfile()
...
- Never bold or italicise any word. The output will be pasted directly into an Excel cell and Markdown symbols will appear as literal characters.`
  ...
  try {
    ...
    if (trimmed.startsWith('NEEDS_MANUAL_REVIEW')) {
      ...
      return {
        answer: explanation || `This question requires a decision or clarification from the ${getActiveProfile().orgName} procurement team.`,
        needsManual: true
      }
    }
...
function generateRfpPdf(rfp: any): Uint8Array {
  ...
  function drawHeaderBand(): string {
    ...
    s += `(${getActiveProfile().orgLocation}) Tj\n`
    // Contact line
    s += `${FONT_REG} 7.5 Tf\n`
    s += `${PW/2 - 70} ${logoMidY - 13} Td\n`
    s += `(${getActiveProfile().procurementEmail}) Tj\n`
    s += `ET\n`
...
Issuing Entity:   ${getActiveProfile().orgName}${rfp?.country_of_issue ? ', ' + rfp.country_of_issue : ''}
...
Best regards,
Procurement & Contracting Department
${getActiveProfile().orgName}
${getActiveProfile().orgLocation}
${getActiveProfile().procurementEmail}
...
apiRouter.post('/admin/set-profile', async (c) => {
  ...
  const DEMO_PIN = (c.env as any)?.DEMO_SWITCH_PIN || process.env.DEMO_SWITCH_PIN || ''
  ...
  try {
    ...
    const { setActiveProfile, markRefreshed } = await import('../profiles/index')
    setActiveProfile(profileId as any)
    markRefreshed()
