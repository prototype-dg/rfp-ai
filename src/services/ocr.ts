...
function getVisionClient(apiKey: string): ImageAnnotatorClient {
  if (!_visionClient) {
...
export async function extractTextFromPdf(
  ...
  const pages_total: number = document.length
  ...
  for await (const page of document) {
    if (pageIdx >= pages_to_process) break
  ...
  console.log(
    `[ocr] done: ${pages_extracted}/${pages_total} pages extracted, ${full_text.length} chars${truncated ? ' (truncated)' : ''}`
  )
